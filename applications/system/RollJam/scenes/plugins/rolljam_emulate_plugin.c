// scenes/plugins/rolljam_emulate_plugin.c

#include "rolljam_emulate_plugin.h"
...
#define TAG "RollJamEmulatePlugin"
...
static const RollJamEmulateHostApi* g_host_api = NULL;

static void emulate_request_nav_pop(RollJamApp* app) {
    app->emulate_nav_pending = EMULATE_NAV_POP;
}

static void emulate_request_nav_after_exit(RollJamApp* app) {
    if(scene_manager_has_previous_scene(app->scene_manager, RollJamSceneStart)) {
        app->emulate_nav_pending = EMULATE_NAV_POP;
    } else {
...
static bool emulate_radio_ready(RollJamApp* app) {
    furi_check(app);
...
static void emu_stop_tx(RollJamApp* app) {
...
static bool emulate_context_try_init_transmitter(RollJamApp* app, EmulateContext* ctx) {
    ...
    if(event->type == InputTypePress) {
        if(event->key == InputKeyBack) {
            view_dispatcher_send_custom_event(
                app->view_dispatcher, RollJamCustomEventEmulateExit);
            return true;
        }
        ...
        if(ctx && ctx->is_transmitting) {
            view_dispatcher_send_custom_event(
                app->view_dispatcher, RollJamCustomEventEmulateStop);
            return true;
        }
        return false;
    }
...
static void plugin_on_enter(void* context) {
    ...
    view_dispatcher_switch_to_view(app->view_dispatcher, RollJamViewAbout);
}
...
#define INVALID_PRESET "Cannot set TX power on this preset."
    ...
    if(app->txrx->txrx_state == RollJamTxRxStateTx) {
        ...
        case RollJamCustomEventEmulateStop:
            FURI_LOG_I(TAG, "Stop event received, txrx_state=%d", app->txrx->txrx_state);
            if(app->txrx->txrx_state == RollJamTxRxStateTx && ctx) {
        ...
        case RollJamCustomEventEmulateExit:
            if(app->txrx->txrx_state == RollJamTxRxStateTx) {
                emu_stop_tx(app);
                if(ctx) {
                    ctx->is_transmitting = false;
...
static void plugin_on_exit(void* context) {
    ...
    if(app->txrx && app->txrx->txrx_state == RollJamTxRxStateTx) {
        ...
        app->txrx->txrx_state = RollJamTxRxStateIDLE;
    } else if(app->txrx && app->txrx->txrx_state != RollJamTxRxStateIDLE) {
...
static void plugin_set_host_api(const RollJamEmulateHostApi* host_api) {
    g_host_api = host_api;
}

static const RollJamEmulatePlugin rolljam_emulate_plugin = {
    .plugin_name = "RollJam Emulate",
...
const FlipperAppPluginDescriptor* rolljam_emulate_plugin_ep(void) {
    return &rolljam_emulate_plugin_descriptor;
}
