...
#define NEXUS_TH_UPLOAD_PAIRS_PER_REPEAT (1 + 36)
#define NEXUS_TH_UPLOAD_MAX_REPEAT       12
#define NEXUS_TH_UPLOAD_MAX_SIZE \
    (NEXUS_TH_UPLOAD_PAIRS_PER_REPEAT * 2 * NEXUS_TH_UPLOAD_MAX_REPEAT)
...
void* ws_protocol_encoder_nexus_th_alloc(SubGhzEnvironment* environment) {
    ...
    instance->encoder.size_upload = NEXUS_TH_UPLOAD_MAX_SIZE;
...
/**
 * Build the PPM upload for one repeat of the 36-bit Nexus-TH frame.
 * Pulse is always te_short; the following gap encodes the bit: short*2 for a
 * 0 bit, te_long (=short*4) for a 1 bit. Each repeat is preceded by a sync
 * gap of te_short*8, matching what ws_protocol_decoder_nexus_th_feed expects.
 */
static bool ws_protocol_encoder_nexus_th_get_upload(WSProtocolEncoderNexus_TH* instance) {
    furi_assert(instance);

    size_t index = 0;
    size_t size_upload = NEXUS_TH_UPLOAD_PAIRS_PER_REPEAT * 2;
    ...
    for(uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
        ...
        instance->encoder.upload[index++] = level_duration_make(
            false,
            bit ? (uint32_t)ws_protocol_nexus_th_const.te_long :
                  (uint32_t)ws_protocol_nexus_th_const.te_short * 2);
    }
...
SubGhzProtocolStatus
    ...
    do {
        ...
        if(instance->generic.data_count_bit != ws_protocol_nexus_th_const.min_count_bit_for_found) {
