...
void ws_view_receiver_info_set_callback(
    ...
    ws_receiver_info->context = context;
}
...
// Only protocols with a working encoder can have fake data sent to them.
static bool ws_view_receiver_info_can_send_fake(const WSReceiverInfoModel* model) {
    return weather_chaos_protocol_supported(furi_string_get_cstr(model->protocol_name));
}
...
void ws_view_receiver_info_set_reading(
    ...
    with_view_model(
        ...
        WSReceiverInfoModel * model,
        {
            model->generic->temp = temp;
            model->generic->humidity = humidity;
...
void ws_view_receiver_info_draw(Canvas* canvas, WSReceiverInfoModel* model) {
    ...
    // Frame shifted up from the original y=38/h=25 to leave clearance for
    // the "Send fake" button hint (12px tall) anchored to the bottom edge.
    elements_bold_rounded_frame(canvas, 0, 28, 127, 20);
    canvas_set_font(canvas, FontPrimary);

    if(!float_is_equal(model->generic->temp, WS_NO_TEMPERATURE)) {
        canvas_draw_icon(canvas, 6, 33, &I_Therm_7x16);
        ...
        canvas_draw_str_aligned(canvas, temp_x1, 37, AlignRight, AlignTop, buffer);
        canvas_draw_circle(canvas, temp_x2, 36, 1);
    }

    if(model->generic->humidity != WS_NO_HUMIDITY) {
        canvas_draw_icon(canvas, 53, 34, &I_Humid_8x13);
        snprintf(buffer, sizeof(buffer), "%d%%", model->generic->humidity);
        canvas_draw_str(canvas, 64, 45, buffer);
    }

    if((int)model->generic->timestamp > 0 && model->curr_ts) {
        int ts_diff = (int)model->curr_ts - (int)model->generic->timestamp;

        canvas_draw_icon(canvas, 91, 36, &I_Timer_11x11);
        ...
        if(ts_diff > 60) {
            ...
            if(model->curr_ts % 2 == 0) {
                canvas_draw_str_aligned(canvas, 105, 41, AlignLeft, AlignCenter, "Old");
            } else {
                if(cnt_min >= 59) {
                    canvas_draw_str_aligned(canvas, 105, 41, AlignLeft, AlignCenter, "Old");
                } else {
                    snprintf(buffer, sizeof(buffer), "%dm", cnt_min);
                    canvas_draw_str_aligned(canvas, 114, 41, AlignCenter, AlignCenter, buffer);
                }
            }

        } else {
            snprintf(buffer, sizeof(buffer), "%d", ts_diff);
            canvas_draw_str_aligned(canvas, 112, 41, AlignCenter, AlignCenter, buffer);
        }
    }

    if(ws_view_receiver_info_can_send_fake(model)) {
        elements_button_left(canvas, "Send fake");
    }
}
...
bool ws_view_receiver_info_input(InputEvent* event, void* context) {
    ...
    if(event->key == InputKeyLeft && event->type == InputTypeShort) {
        ...
        with_view_model(
            ws_receiver_info->view,
            WSReceiverInfoModel * model,
            { can_send = ws_view_receiver_info_can_send_fake(model); },
            false);
        if(can_send && ws_receiver_info->callback) {
            ws_receiver_info->callback(
                WSCustomEventViewReceiverInfoSendFake, ws_receiver_info->context);
        }
    }
